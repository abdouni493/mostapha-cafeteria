...
/**
 * Every session of every cafétéria, or `null` when the table is not there yet — the
...
export async function loadBizSessions(): Promise<Record<ModuleKey, BizSession[]> | null> {
  try {
    ...
    // Une clé par cafétéria : elles sont dynamiques, on prend ce que la base
    // renvoie et l'appelant ne lira que les cafétérias qu'il connaît.
    const out: Record<ModuleKey, BizSession[]> = {};
    ((data || []) as BizSessionRow[]).forEach(r => {
      const key = r.module_key as ModuleKey;
      if (key) (out[key] ||= []).push(rowToSession(r));
    });
    return out;
  } catch {
    return null;
