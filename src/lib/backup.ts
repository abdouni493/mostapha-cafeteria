...
import { MERGE_COLLECTIONS,  STAMP } from './bizSync';
...
export const BACKUP_TABLES: BackupTableSpec[] = [
  // L'enseigne
  { table: 'store_settings',   label: "Reglages de l'enseigne" },
  // Le profil d'un administrateur, PAS son compte de connexion : mots de passe et
  // comptes vivent dans le schema `auth` de Supabase, hors d'atteinte de
  // l'application. La restauration de cette table n'aboutit donc que si le compte
  // existe encore, la cle etrangere l'exige et le rapport le dira.
  { table: 'admin_profiles',   label: 'Profils administrateurs (hors mots de passe)' },
  { table: 'module_workers',   label: 'Employes de cafeteria (hors mots de passe)' },
  // Le coffre general
  { table: 'general_cash',     label: 'Caisse generale' },
  // Les cafeterias
  { table: 'biz_products',     label: 'Catalogue des cafeterias' },
  { table: 'biz_sessions',     label: 'Sessions de caisse' },
  // Journal
  { table: 'activity_log',     label: "Journal d'activite" },
];
...
export function mergeBackupIntoBiz(current: any, backup: any): BizState {
  const curMods = (current?.modules || {}) as Record<string, any>;
  const bakMods = (backup?.modules || {}) as Record<string, any>;

  /**
   * Le REGISTRE d'abord : une cafétéria que seul le fichier connaît doit
   * revenir, sinon ses données seraient restaurées sans que rien ne les affiche.
   * Une cafétéria d'aujourd'hui absente du fichier reste, elle, en place.
   */
  const cafeterias: any[] = [...(Array.isArray(current?.cafeterias) ? current.cafeterias : [])];
  for (const c of (Array.isArray(backup?.cafeterias) ? backup.cafeterias : [])) {
    if (c?.id && !cafeterias.some(x => x.id === c.id)) cafeterias.push(c);
  }

  const modules: any = {};
  const keys = new Set<string>([...Object.keys(curMods), ...Object.keys(bakMods), ...cafeterias.map(c => c.id)]);

  for (const key of keys) {
    const cur = (curMods[key] || {}) as any;
    const bak = (bakMods[key] || {}) as any;
    const mod: any = { ...cur };
  ...
  return { cafeterias, modules } as BizState;
}
