...
import { SyncModuleState, mergeRows } from './bizSync';
...
export async function loadBizProducts(): Promise<ProductsLoadResult> {
  try {
    ...
    // Les clés sont dynamiques (une par cafétéria) : on accepte celles que la
    // base renvoie, et l'appelant ignorera celles qu'il ne connaît pas encore.
    const out: Record<ModuleKey, BizProduct[]> = {};
    for (const row of (data || []) as BizProductRow[]) {
      const key = row.module_key as ModuleKey;
      if (!key) continue;
      const product = rowToProduct(row);
      if (!product) continue;
      (out[key] ||= []).push(product);
    }
...
export function mergeRemoteProducts(
  ...
  const modules: Record<string, any> = { ...state.modules };
  let changed = false;

  for (const key of Object.keys(state.modules || {})) {
    const mod = state.modules[key] as SyncModuleState | undefined;
    if (!mod) continue;
    ...
    changed = true;
    modules[key] = { ...mod, products: merged };
  }

  return changed ? ({ ...state, modules } as BizState) : state;
}
