 ...
 * ──────────────────────────────────────────────────────────────────────────────
 */
import { ModuleState, ModuleKey, MODULES, isReversedSale } from './bizConfig';
...
// ─── Résultats ───────────────────────────────────────────────────────────────
export type ProductKind = 'catalogue' | 'production' | 'fiche' | 'autre';
...
/**
 * Regroupe les produits par CATÉGORIE. Un produit sans catégorie tombe dans
 * « Sans catégorie » plutôt que d'être écarté : un rayon mal renseigné doit se
 * voir dans le rapport, c'est ce qui pousse à le corriger.
 */
function groupByCategory(products: ProductAnalytics[]): CategoryAnalytics[] {
  const acc = new Map<string, CategoryAnalytics>();
  for (const p of products) {
    const name = p.category || 'Sans catégorie';
    const row = acc.get(name) || { name, qty: 0, revenue: 0, cost: 0, gain: 0, share: 0, products: 0 };
    row.qty += p.qty;
...
/** Les totaux de la période, tirés de la même série que les graphiques. */
function totalsOf(points: TimePoint[], products: ProductAnalytics[]): PartAnalytics['totals'] {
  const sum = (f: (p: TimePoint) => number) => points.reduce((s, p) => s + f(p), 0);
  const sales = sum(p => p.revenue);
  const cost = sum(p => p.cost);
  const purchases = sum(p => p.purchases);
  const expenses = sum(p => p.expenses);
  return {
    sales,
    cost,
    margin: sales - cost,
    purchases,
    expenses,
    // Le NET retranche les charges de la marge, pas du chiffre d'affaires :
    // soustraire les dépenses d'un CA donnerait un « résultat » qui ignore
    // purement et simplement le coût de la marchandise vendue.
    net: sales - cost - expenses,
    qty: sum(p => p.qty),
    operations: sum(p => p.count),
