...
const KIND_ICON: Record<MovementKind, React.ElementType> = {
  purchase: ShoppingCart, sale: Receipt, production: Beaker, destruction: Flame,
};
const KIND_TONE: Record<MovementKind, 'success' | 'info' | 'warning' | 'danger' | 'neutral'> = {
  purchase: 'success', sale: 'info', production: 'neutral', destruction: 'danger',
};
...
export default function ProductHistoryModal({ product, state, settings, onClose }: {
  ...
  const shown = useMemo(() => {
    ...
    const sells = rows.filter(m => (m.kind === 'sale') && !m.canceled);
    return {
