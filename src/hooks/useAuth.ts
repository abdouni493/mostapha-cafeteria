 ...
 *   - `admin`         : voit toutes les cafeterias et regle l'application ;
 *   - `module_worker` : employe d'UNE cafeteria, limite a ce qu'on lui a coche.
 *
 * `module_worker` est aussi le role de REPLI : quand la recherche du role
 * echoue ou traine, on entre avec le MOINS de droits possible. Retomber sur
 * `admin` en cas de panne reseau aurait ouvert toute l'application a n'importe
 * quel compte des que la base ralentit.
 */
export type UserRole = 'admin' | 'module_worker';
 ...
 */
const ROLE_CACHE_KEY = 'altech.cafeteria.role';

const VALID_ROLES: UserRole[] = ['admin', 'module_worker'];
...
export function useAuth() {
  ...
  const releaseNow = () => {
    ...
    setAuth(prev => (prev.isLoading ? {
      ...prev,
      userRole: prev.isAuthenticated && !roleResolved.current ? 'module_worker' : prev.userRole,
      isLoading: false,
    } : prev));
  };
  ...
  // Falls back to 'module_worker' (least privilege) when the lookup fails.
  // when the RPC doesn't exist yet (pre-migration environments).
  /**
   * Le role du compte connecte.
   *
   * `get_my_role` est une fonction SECURITY DEFINER : elle repond en un seul
   * aller-retour et sans exposer les tables. Les deux lectures directes qui la
   * doublent ne sont pas de la redondance decorative : elles servent quand la
   * fonction n'existe pas encore (base fraiche, schema pas encore applique).
   *
   * En cas d'echec total, le repli est `module_worker` : le compte entre avec
   * le MINIMUM de droits, et sans cafeteria attribuee il ne voit rien du tout.
   * C'est volontaire — une panne de reseau ne doit jamais promouvoir quelqu'un
   * administrateur.
   */
  const fetchRole = async (_userId: string): Promise<UserRole> => {
    const lookupTables = async (): Promise<UserRole> => {
    ...
    try {
      ...
      if (error) {
        console.warn('[useAuth] get_my_role:', error.message);
        return await lookupTables();
      }
      // Une revendication « admin » est VERIFIEE contre `admin_profiles` avant
      // d'etre accordee : c'est la seule table qui fait foi pour ce role.
      if (data === 'admin') return await lookupTables();
      if (data === 'module_worker') return 'module_worker';
      return await lookupTables();
    } catch (err) {
      console.error('[useAuth] fetchRole:', err);
      try { return await lookupTables(); } catch { return 'module_worker'; }
    }
  };
    ...
    profileFetchRef.current = withTimeout(lookup, ROLE_LOOKUP_TIMEOUT_MS, 'role lookup')
      ...
      .catch((err: Error) => {
        ...
        if (mountedRef.current) {
          setPhase(null);
          setAuth(prev => (prev.isLoading ? { ...prev, userRole: 'module_worker', isLoading: false } : prev));
        }
      });
  };
