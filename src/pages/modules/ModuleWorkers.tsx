...
import { useBiz, useActiveCafeterias } from '@/src/store/BizContext';
...
export default function ModuleWorkers({ moduleKey }: { moduleKey: ModuleKey }) {
  ...
  const filtered = workers.filter(w => matchesSearch(search, w.name, w.roleName, w.phone, w.cin));
...
function WorkerForm({ moduleKey, initial, onClose }: { moduleKey: ModuleKey; initial: BizWorker | null; onClose: () => void }) {
  ...
  const [f, setF] = useState<Partial<BizWorker>>(initial || {
    name: '', birthday: '', cin: '', phone: '', roleName: '', paid: true,
    salaryType: 'mois', salaryAmount: 0,
    workDays: DEFAULT_WORK_DAYS, cnasDate: '',
    hasAccount: false, email: '', username: '', password: '',
    startDate: new Date().toISOString().split('T')[0],
  });
  /**
   * ─── LA CAFÉTÉRIA DE CET EMPLOYÉ ────────────────────────────────────────
   * Un employé appartient à UNE cafétéria, et c'est ce rattachement qui décide
   * de tout ce qu'il verra : son stock, ses ventes, ses clients, sa caisse. Il
   * n'aura JAMAIS accès à ceux d'une autre cafétéria, quelles que soient les
   * permissions qu'on lui coche ensuite (voir `useBizPermission`).
   *
   * À la création, il est rattaché à la cafétéria depuis laquelle on le crée —
   * mais on peut en choisir une autre sans changer d'écran. En modification, ce
   * choix disparaît : déplacer un employé reviendrait à couper son historique
   * de paie en deux, il faut le recréer dans l'autre cafétéria.
   */
  const [targetCaf, setTargetCaf] = useState<ModuleKey>(moduleKey);
  const targetBiz = useBiz(targetCaf);
  ...
  const save = async () => {
    ...
    if (f.hasAccount) {
      ...
      const res = await provisionModuleWorkerAccount({ action: 'delete', moduleKey: targetCaf, workerId });
    ...
    // La fiche part dans la cafétéria CHOISIE, qui n'est pas forcément celle
    // de l'écran courant : c'est ce qui permet de créer l'équipe des trois
    // comptoirs sans changer de page trois fois.
    if (isEdit) biz.update('workers', worker); else targetBiz.add('workers', worker);
  ...
  return (
    <Modal open onClose={onClose} icon={UsersRound} size="lg" title={isEdit ? "Modifier l'employé" : 'Nouvel employé'} subtitle="Informations & compte"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        {/* ── À quelle cafétéria appartient-il ? ────────────────────── */}
        {!isEdit && cafeterias.length > 1 && (
          <div className="sm:col-span-2 rounded-2xl border border-[#E7C9A9] bg-[#F5E7D8]/60 p-4">
            <p className="text-[11px] font-black uppercase tracking-wider text-[#8A5A2B]">Cafétéria de cet employé</p>
            <p className="text-[10.5px] text-[#8A5A2B]/80 mb-2.5 leading-relaxed">
              Il ne verra QUE cette cafétéria : son stock, ses ventes, ses clients et sa caisse.
              Les autres comptoirs lui resteront invisibles.
            </p>
            <div className="flex flex-wrap gap-1.5">
              {cafeterias.map(c => (
                <button key={c.id} type="button" onClick={() => setTargetCaf(c.id)}
                  className={`px-3 py-1.5 rounded-xl text-[12px] font-bold border transition-all ${
                    targetCaf === c.id ? 'text-white shadow' : 'bg-white text-[#7A6A5C] border-[#E2D3C4] hover:border-[#D4A373]'
                  }`}
                  style={targetCaf === c.id ? { background: c.color || '#6F4E37', borderColor: c.color || '#6F4E37' } : undefined}>
                  {c.emoji || '☕'} {c.name}
                </button>
              ))}
            </div>
          ...
          {f.paid && (
            <>
              <div className="mt-3 grid grid-cols-2 gap-3">
                ...
                <Field label={f.salaryType === 'jour' ? 'Montant (DA / jour)' : 'Montant (DA / mois)'}>
                  <Input type="number" value={f.salaryAmount ?? 0} onChange={e => set('salaryAmount', e.target.value)} />
                </Field>
              </div>
              {f.salaryType === 'jour' && (
...
function PaymentModal({ moduleKey, worker, onClose }: { moduleKey: ModuleKey; worker: BizWorker; onClose: () => void }) {
  ...
  const onConfirm = (res: WorkerPaymentResult) => {
    const payment: BizWorkerPayment = {
      ...
      worksTotal: res.worksTotal || undefined,
...
function PrintPaymentsModal({ moduleKey, worker, onClose }: {
  ...
  const print = () => {
    printInvoice({
      ...
      info: [
        { label: 'Cafétéria', value: cfg.label },
        { label: 'Fonction', value: worker.roleName },
        { label: 'Période', value: `${formatDate(from)} → ${formatDate(to)}` },
        { label: 'Mode de paie', value: worker.salaryType === 'jour' ? 'Journalier' : 'Mensuel' },
      ],
      items: rows.map(p => ({
