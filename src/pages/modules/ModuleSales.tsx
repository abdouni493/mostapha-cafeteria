...
import { useBiz, useBizAll, useActiveCafeterias } from '@/src/store/BizContext';
...
export default function ModuleSales({ moduleKey }: { moduleKey: ModuleKey }) {
  ...
  const basePerm = useBizPermission(moduleKey, 'sales');
  /**
   * Vue comparée : LECTURE SEULE. Encaisser, corriger ou rembourser un ticket
   * touche la caisse et le stock de SA cafétéria — le faire depuis l'écran d'un
   * autre comptoir écrirait au mauvais endroit.
   */
  ...
  /**
   * ─── LES VENTES DE TOUTES LES CAFÉTÉRIAS ─────────────────────────────────
   * Le même historique, mais mis bout à bout, chaque ticket portant la pastille
   * de son comptoir. En lecture seule : encaisser, corriger ou rembourser une
   * vente se fait dans SA cafétéria, où la caisse et le stock suivent.
   */
  const bizAll = useBizAll();
  const cafeterias = useActiveCafeterias();

  const salesRows = useMemo(() => {
    if (!allCafeterias) return sales.map(x => ({ sale: x, cafId: moduleKey }));
    return cafeterias.flatMap(c =>
      (bizAll.modules[c.id]?.sales || []).map(x => ({ sale: x, cafId: c.id })));
  }, [allCafeterias, sales, moduleKey, cafeterias, bizAll]);

  const cafOf = useMemo(() => {
    const m = new Map<string, string>();
    for (const r of salesRows) m.set(r.sale.id, r.cafId);
    return m;
  }, [salesRows]);

  const filtered = useMemo(() => {
    return salesRows.map(r => r.sale).filter(s => {
  ...
  }, [salesRows, clients, search, period, from, to]);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <CafeteriaSwitcher current={moduleKey}
        compare={{ active: allCafeterias, onChange: setAllCafeterias, label: 'Toutes les ventes' }} />
            ...
            <GlassCard key={s.id}>
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  <h3 className="font-black text-slate-800">{s.ref}</h3>
                  <p className="text-xs text-slate-400 flex items-center gap-1 mt-0.5"><User className="w-3 h-3" />{s.clientName}</p>
                  {/* Deux comptoirs numerotent leurs tickets separement : la pastille
                      dit lequel a encaisse celui-ci. */}
                  {allCafeterias && <span className="inline-block mt-1"><CafeteriaTag id={cafOf.get(s.id) || moduleKey} /></span>}
                </div>
                <StatusBadge sale={s} />
              </div>
            ...
            <tr key={s.id}>
              <td className="table-cell font-bold">
                <span className="flex items-center gap-1.5 flex-wrap">
                  {s.ref}
                  {allCafeterias && <CafeteriaTag id={cafOf.get(s.id) || moduleKey} />}
                </span>
              </td>
