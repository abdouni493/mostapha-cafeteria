...
import { useBiz, useBizSync, useBizProductsSync, useBizAll, useActiveCafeterias } from '@/src/store/BizContext';
import { useBizPermission, useAppState } from '@/src/store/AppContext';
import CafeteriaSwitcher, { CafeteriaTag } from '@/src/components/biz/CafeteriaSwitcher';
import {
...
export default function ModuleStock({ moduleKey }: { moduleKey: ModuleKey }) {
  ...
  const basePerm = useBizPermission(moduleKey, 'stock');
  /**
   * ─── POURQUOI LA VUE COMPARÉE EST EN LECTURE SEULE ───────────────────────
   * Les boutons de cette page écrivent dans la cafétéria de l'URL. Les laisser
   * actifs sur une fiche appartenant à un AUTRE comptoir modifierait donc le
   * mauvais catalogue — en silence. On coupe les actions plutôt que de risquer
   * ça : pour agir sur une fiche, on ouvre sa cafétéria.
   */
  const [allCafeterias, setAllCafeterias] = useState(false);
  const perm = allCafeterias ? { voir: true, creer: false, modifier: false, supprimer: false } : basePerm;
  ...
  /**
   * ─── LA VUE « TOUTES LES CAFÉTÉRIAS » ────────────────────────────────────
   *
   * Chaque cafétéria tient son propre catalogue : le même « Café allongé » y est
   * une fiche DIFFÉRENTE, avec son stock et son prix. C'est voulu — un comptoir
   * ne doit pas voir son stock bouger parce que le voisin a vendu.
   *
   * Mais le gérant, lui, a besoin de la question inverse : « où en est le café
   * PARTOUT ? ». Cette vue met les catalogues côte à côte, chaque ligne portant
   * la pastille de sa cafétéria. Elle est en LECTURE SEULE : modifier une fiche
   * se fait dans sa cafétéria, là où l'on voit son historique et ses achats.
   */
  const bizAll = useBizAll();
  const cafeterias = useActiveCafeterias();

  /** Le catalogue affiché : celui de cette cafétéria, ou celui de toutes. */
  const catalogueRows = useMemo(() => {
    if (!allCafeterias) return products.map(p => ({ product: p, cafId: moduleKey }));
    return cafeterias.flatMap(c =>
      (bizAll.modules[c.id]?.products || []).map(p => ({ product: p, cafId: c.id })));
  }, [allCafeterias, products, moduleKey, cafeterias, bizAll]);
  ...
  }, [catalogueRows, search, cat, mrq, nature]);

  /** À quelle cafétéria appartient une fiche — pour la pastille des listes. */
  const cafOf = useMemo(() => {
    const m = new Map<string, string>();
    for (const r of catalogueRows) m.set(r.product.id, r.cafId);
    return m;
  }, [catalogueRows]);
    ...
    <div className="space-y-6 animate-fade-in">
      <PageHeader icon={Package} title="Gestion de stock"
        ...
        actions={perm.creer && !allCafeterias
          ? <button className="btn-primary" onClick={openNew}><Plus className="w-4 h-4" /> Nouveau produit</button>
          : undefined} />

      <CafeteriaSwitcher current={moduleKey}
        compare={{ active: allCafeterias, onChange: setAllCafeterias, label: 'Comparer les catalogues' }} />

      {allCafeterias && (
        <div className="rounded-2xl border border-[#E7C9A9] bg-[#F5E7D8]/60 px-4 py-3 text-[12px] text-[#8A5A2B] leading-relaxed">
          Vous voyez le catalogue de <b>{cafeterias.length} cafétérias</b> à la fois. Chaque fiche
          appartient à SA cafétéria : pour la modifier, réapprovisionner ou consulter son
          historique, ouvrez d'abord sa cafétéria ci-dessus.
        </div>
      )}
            ...
            <GlassCard key={p.id}>
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  ...
                  {/* En vue « toutes les cafeterias », chaque fiche dit d'ou elle vient :
                      sans cela, deux « Cafe allonge » cote a cote seraient indiscernables. */}
                  {allCafeterias && <span className="inline-block mt-1"><CafeteriaTag id={cafOf.get(p.id) || moduleKey} /></span>}
                </div>
                {negative(p)
            ...
            <tr key={p.id}>
              <td className="table-cell">
                <div className="font-bold text-slate-700 flex items-center gap-1.5 flex-wrap">
                  {p.name}
                  {allCafeterias && <CafeteriaTag id={cafOf.get(p.id) || moduleKey} />}
                  {p.isRawMaterial && (
